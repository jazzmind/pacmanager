...
export class Store {
  ...
  publish(p,appId){const app=this.access(p,appId,true);if(app.build?.status!=='ready'||app.build.revision!==app.revision)fail(409,'Build the current draft before publishing');app.release={number:(app.release?.number||0)+1,publishedAt:now(),revision:app.revision,mode:app.build.mode,result:structuredClone(app.build.result)};this.audit(app,'published release '+app.release.number,p);this.save();return app.release;}
